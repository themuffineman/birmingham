...
const LeadCard = ({platform = 'google', emails, name, url}) => {
    async function createLead(name, email){
        try {
            await fetch('/api/create-lead', {method: "POST" , body: JSON.strinigfy({name, email})})
  ...
  return (
    <div className='grid grid-row-1 grid-flow-col justify-between items-center justify-items-center ring ring-slate-500 w-[70rem] rounded-md p-4'>
        ...
        <div className='flex gap-3 items-center '>
            <button onClick={()=> createLead(name, emails[0])} className="p-2 w-max rounded-md hover:ring active:translate-y-1 transition-transform hover:ring-black text-white bg-yellow-500 hover:text-black hover:bg-yellow-300 text-base font-semibold">Add</button>
